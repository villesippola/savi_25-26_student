...
def main():
    ...
    for group_idx in group_idxs:
        group_size = len(list(locate(labels, lambda x: x == group_idx)))
        if group_size > biggest_group_size:
            biggest_group_size = group_size
            biggest_group_idx = group_idx

    print(f'Biggest group index: {biggest_group_idx}, size: {biggest_group_size}')

    # ------------------------------------
    # Extract only the biggest cluster
    # ------------------------------------
    biggest_group_points_idxs = list(locate(labels, lambda x: x == biggest_group_idx))
    biggest_group_point_cloud = downsampled_point_cloud.select_by_index(biggest_group_points_idxs, invert=False)
    biggest_group_point_cloud.paint_uniform_color([1, 0, 0]) # Paint the biggest cluster in red

    # ------------------------------------
    # Visualize only the biggest object
    # ------------------------------------
    # Create entities list of objects to draw
    entities = [axes_mesh, biggest_group_point_cloud]
